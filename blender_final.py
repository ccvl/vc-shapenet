...
import texture_util
texture_util.set_search_path('./textures')

def set_camera():
    pass # Do nothing, keep the camera pose in blank.blend

def set_lighting():
    pass # Do nothing, keep the lighting from blank.blend

def get_meshes():
    # Iterate all actors in the scene and apply action to the actor
    return bpy.data.objects

def print_name(actor):
    # Print the name of an actor
    msg = 'Name: %s, Type: %s' % (actor.name, actor.type)
    print(msg)

def apply_texture(mesh, texture_id):
    texture = texture_util.get_by_id(texture_id)
    texture_util.apply_texture(mesh, texture, scale = 0.1)
def debug_info():
    print('The loaded texture bank:')
    print(texture.texture_bank)
...
g_syn_light_environment_energy_lowbound = 0.1
g_syn_light_environment_energy_highbound = 1

#for param in view_params:
for i in range(1000):
    azimuth_deg = np.random.randint(360)
    elevation_deg = np.random.randint(-90,90)%360
    theta_deg = 0
    rho = 2
    ...
    texture_ind = np.random.randint(len(texture_util.texture_bank)) 
    [apply_texture(mesh, texture_ind) for mesh in meshes]
    syn_image_file = './%s_%s_%d_a%03d_e%03d_t%03d_d%03d.jpg' % (shape_synset, shape_md5, texture_ind,round(azimuth_deg), round(elevation_deg), round(theta_deg), round(rho))
