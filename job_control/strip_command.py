...
with open('render_final_commd.txt') as f:
    command_lines = f.readlines()
commands = [l.split(' ') for l in command_lines]
obj_categories = [v[8] for v in commands]
obj_instances = [v[9] for v in commands]
assert len(obj_categories[0]) == 8 
assert len(obj_instances[0]) == 32
